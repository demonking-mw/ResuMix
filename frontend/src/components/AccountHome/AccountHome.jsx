...
const AccountHome = () => {
	...
	// Helper function to get card CSS classes based on status
	const getCardClasses = (stepType, baseClasses = "step-card") => {
		let classes = baseClasses;

		// Check if generate step should be disabled (red status)
		if (stepType === "generate" && userStatus?.generate_status === "r") {
			classes += " step-card-disabled";
		}
		// Check if tweak step should be disabled (when resume status is red)
		else if (stepType === "tweak" && userStatus?.resume_state === "r") {
			classes += " step-card-disabled";
		} else {
			classes += " clickable-card";
		}

		// Add accent based on what needs attention
		if (
			stepType === "resume" &&
			(userStatus?.resume_state === "r" || userStatus?.resume_state === "o")
		) {
			classes += " step-card-accent-urgent";
		} else if (
			stepType === "tweak" &&
			userStatus?.tweak_status === "o" &&
			(userStatus?.resume_state === "y" || userStatus?.resume_state === "g") &&
			userStatus?.generate_status !== "g"
		) {
			classes += " step-card-accent-urgent";
		} else if (stepType === "generate" && userStatus?.generate_status === "g") {
			classes += " step-card-accent-urgent";
		}
	...
	// Helper function to handle card clicks with status check
	const handleCardClickWithStatusCheck = (destination, stepType) => {
		// Disable click for generate step if status is red
		if (stepType === "generate" && userStatus?.generate_status === "r") {
			return;
		}
		// Disable click for tweak step if resume status is red
		if (stepType === "tweak" && userStatus?.resume_state === "r") {
			return;
		}
			...
			<main className="account-main">
				<div className="workflow-container">
					...
					<div className="workflow-step">
						...
						<div
							className={getCardClasses("resume")}
							onClick={() =>
								handleCardClickWithStatusCheck(
									"/master-resume?mode=edit",
									"resume"
								)
							}
						>
					...
					<div className="workflow-step">
						...
						<div
							className={getCardClasses("tweak")}
							onClick={() =>
								handleCardClickWithStatusCheck(
									"/master-resume?mode=parameters-only",
									"tweak"
								)
							}
						>
					...
					<div className="workflow-step">
						...
						<div
							className={getCardClasses("generate")}
							onClick={() =>
								handleCardClickWithStatusCheck("/optimize", "generate")
							}
						>
							<div className="step-content">
