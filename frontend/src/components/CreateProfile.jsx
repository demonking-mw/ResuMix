  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.data) {
        const detail =
          err.response.data.detail?.status || err.response.data.message;
        if (detail === "uid or email unique violation") {
          setMessage("User ID or email already exists");
        } else {
          setMessage(`Error: ${detail}`);
      ...
      if (currentTime < iat - timeBuffer || currentTime > exp + timeBuffer) {
        ...
        try {
          ...
          if (response.data.status) {
            ...
            // Use auth context login method with Google user data and reauth JWT
            const userData = {
            ...
            const detail =
              response.data.detail?.status || response.data.message;
